	...
	public void run(){
		...
		while (true){
				...
				if (msgIn != null){
					...
					msgOut = "Current directory is " + currDir;
					...
					else if(msgIn.substring(0,4).equals("GET ")){
						String fileName = "";
						fileName = msgIn.substring(4);
						...
						if (file.exists()) {
							...
							if (numOfBytes == -1) {
								...
								msgOut = "***INCOMING FILE:" + numOfBytes + "-" + fileName+ "***";
								check = conn.sendMessage(msgOut);
								if(!check) System.out.println("Error.");
								numOfBytes = conn.sendBytes(file);
								msgOut = "Copy successful.";
							}
							
						}
						else msgOut = "File you want to GET is not found.";
					}
					else{
